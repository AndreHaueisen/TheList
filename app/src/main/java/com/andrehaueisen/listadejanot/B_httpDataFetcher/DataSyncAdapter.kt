 ...
 */
class DataSyncAdapter(val mJsoupConnection: Connection, context: Context) : AbstractThreadedSyncAdapter(context, false, false) {

    val LOG_TAG : String = DataSyncAdapter::class.java.simpleName
    ...
    override fun onPerformSync(account: Account?, extras: Bundle?, authority: String?, provider: ContentProviderClient?, syncResult: SyncResult?) {
        ...
        savePoliticiansToDatabase()
                .subscribe(object:SingleObserver<Boolean>{
                    override fun onSuccess(wereAllRowsInserted: Boolean) {
                        if(wereAllRowsInserted) {
                            Log.i(LOG_TAG, "Success! 594 rows inserted")
                        }else{
                            Log.e(LOG_TAG, "Failed to refresh data")
                        }
                        //mDisposable.dispose()
                    }
    ...
    private fun savePoliticiansToDatabase() : Single<Boolean> {
